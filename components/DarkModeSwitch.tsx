...
const DarkModeSwitchInner: FC<BoxProps> = (boxProps) => {
  ...
  return (
    <Box
      as="button"
      bg={colorMode.background({ colorMode: current })}
      color={colorMode.body({ colorMode: current })}
      _hover={{ bg: colorMode.tint({ colorMode: current }) }}
      {...boxProps}
    >
      {current === 'dark' ? <Icons.MoonIcon /> : <Icons.SunIcon />}
    </Box>
  )
}
